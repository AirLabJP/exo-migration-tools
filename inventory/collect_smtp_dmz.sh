...
if command -v postconf >/dev/null 2>&1; then
  ...
  if [ -d /etc/postfix ]; then
    tar -czf "$OUTDIR/etc_postfix.tgz" -C /etc postfix 2>/dev/null || true
    echo "  → 設定アーカイブ保存: etc_postfix.tgz"
  fi
...
# --- Exim ---
  ...
  if [ -d /etc/exim4 ]; then
    tar -czf "$OUTDIR/etc_exim4.tgz" -C /etc exim4 2>/dev/null || true
    echo "  → 設定アーカイブ保存: etc_exim4.tgz"
  fi
...
# --- Sendmail ---
  ...
  if [ -d /etc/mail ]; then
    tar -czf "$OUTDIR/etc_mail.tgz" -C /etc mail 2>/dev/null || true
    echo "  → 設定アーカイブ保存: etc_mail.tgz"
  fi

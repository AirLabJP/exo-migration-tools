...
Write-Host "2. Windows Server 2022をインストール（Desktop Experienceを選択）"
Write-Host "3. DC02のネットワーク設定でDNSをDC01のIPに設定"
Write-Host "4. DC02で以下のコマンドを実行:"
Write-Host "   Install-WindowsFeature -Name AD-Domain-Services -IncludeManagementTools"
Write-Host "   Install-ADDSDomainController `"
Write-Host "     -DomainName `"$DomainName`" `"
Write-Host "     -SafeModeAdministratorPassword `$SafeModePassword `"
Write-Host "     -Credential (Get-Credential -UserName `"$NetBIOSName\Administrator`" -Message `"DC01の管理者資格情報を入力`") `"
Write-Host "     -Force"
